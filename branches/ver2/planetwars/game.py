...
from planetwars.exceptions import GameRulesError
import player
...
class Game(object):
    ...
    def send_fleet(self, source_id, destination_id, ship_count):
        if not ship_count or ship_count == 0:
            raise GameRulesError("We should not send 0 ships")
        if source_id == destination_id:
            raise GameRulesError("Sending ships from %d to itself is prohibited" % source_id)
        source = self.universe.get_planet_by_id(source_id)
        if source and source.owner != player.ME:
            raise GameRulesError("Sending ships from enemy planet %s is prohibited" % source)
