...
from planetwars.orderchain import Strategies
...
class Universe(object):
    ...
    @property
    def max_planet_distance(self):
        max_v = 0
        for k in self._dist_cache:
            v = self._dist_cache[k]
            if v > max_v:
                max_v = v
        return max_v
    ...
    @property
    def my_and_neutral_planets(self):
        """Returns all planets belonging to ME or to NOBODY."""
        return self.find_planets(owner=player.ME_AND_NOBODY)
    ...
    def send_fleet(self, source, destination, ship_count):
        ...
        msg = "Sending fleet of %d from %s to %s." % (ship_count, source, destination)
        log.debug(msg)

        if ship_count <= 0:
            raise NoMoreShipsError(msg)
        if isinstance(destination, set):
            raise Exception("Sending to set is not supported any more")
